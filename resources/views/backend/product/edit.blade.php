...
<div class="card">
    <h5 class="card-header">Добваить товар</h5>
    <div class="card-body">
      <form method="post" action="{{route('product.update',$product->id)}}">
        ...
        <div class="form-group">
          <label for="summary" class="col-form-label">Краткое описание <span class="text-danger">*</span></label>
        ...
        <div class="form-group">
          <label for="is_featured">Является избранным</label><br>
          <input type="checkbox" name='is_featured' id='is_featured' value='{{$product->is_featured}}' {{(($product->is_featured) ? 'checked' : '')}}> Да                        
        </div>
              {{-- {{$categories}} --}}
        ...
        <div class="form-group">
          <label for="price" class="col-form-label">Стоимость (BYN) <span class="text-danger">*</span></label>
          <input id="price" type="number" name="price" placeholder="Введите стоимость"  value="{{$product->price}}" class="form-control">
          @error('price')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
        ...
        <div class="form-group">
          ...
          <select name="condition" class="form-control">
              <option value="">--Выберете состояние--</option>
              <option value="default" {{(($product->condition=='default')? 'selected':'')}}>Default</option>
              <option value="new" {{(($product->condition=='new')? 'selected':'')}}>Новинка</option>
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            ...
            <option value="inactive" {{(($product->status=='inactive')? 'selected' : '')}}>Не активный</option>
        </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
