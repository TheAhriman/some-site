...
<div class="card">
    <h5 class="card-header">Добавить публикацию</h5>
    <div class="card-body">
      <form method="post" action="{{route('post.update',$post->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($post->status=='active')? 'selected' : '')}}>Активный</option>
            <option value="inactive" {{(($post->status=='inactive')? 'selected' : '')}}>Не активный</option>
        </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
